...
import { canAuthorizeConsulting, EXPECTED_CONSULTING_CLIENT_ID } from '@/lib/oauth/server';
...
export async function POST(request: Request) {
  ...
  const allowedClient = Boolean(EXPECTED_CONSULTING_CLIENT_ID) && details.client.id === EXPECTED_CONSULTING_CLIENT_ID;
  const entitled = allowedClient && await canAuthorizeConsulting(supabase);
  const approve = decision === 'approve' && entitled;
