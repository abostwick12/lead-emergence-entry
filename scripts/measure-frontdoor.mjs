...
for (const mobile of [false,true]) {
  ...
  await page.addInitScript(()=>{window.__landingMetrics={lcp:0,lcpElement:'',lcpUrl:'',lcpSize:0,longTasks:[]};new PerformanceObserver(list=>{const entry=list.getEntries().at(-1);window.__landingMetrics.lcp=entry.startTime;window.__landingMetrics.lcpElement=entry.element?.outerHTML?.slice(0,240)??'';window.__landingMetrics.lcpUrl=entry.url??'';window.__landingMetrics.lcpSize=entry.size??0;}).observe({type:'largest-contentful-paint',buffered:true});new PerformanceObserver(list=>{window.__landingMetrics.longTasks.push(...list.getEntries().map(entry=>entry.duration));}).observe({type:'longtask',buffered:true});});
  ...
  const initial=await page.evaluate(()=>({lcpMs:window.__landingMetrics.lcp,lcpElement:window.__landingMetrics.lcpElement,lcpUrl:window.__landingMetrics.lcpUrl,lcpSize:window.__landingMetrics.lcpSize,initialEncodedBytes:performance.getEntriesByType('resource').reduce((n,r)=>n+r.encodedBodySize,0)+performance.getEntriesByType('navigation')[0].encodedBodySize,resourceCount:performance.getEntriesByType('resource').length,longTasks:window.__landingMetrics.longTasks,overflow:document.documentElement.scrollWidth>innerWidth}));
